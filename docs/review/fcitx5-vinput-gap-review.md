...
9. OpenAI-compatible post-processing and command mode parity; endpoint URL/header building with redacted request diagnostics, prompt-file loading, whitespace-tolerant legacy interpolation, provider selection, candidate parsing/response-payload mapping, command-mode OpenAI fallback ordering, protected `extra_body` merging, pure request-body assembly with XML fallback/constraints plus command selected-text XML, and recent-input context-prefix/read/write/truncate helpers are now pinned, while concrete HTTP transport and runtime provider wiring remain.
