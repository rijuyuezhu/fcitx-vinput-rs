...
- Config can parse the current legacy default config and has extensive validation tests.
...
Rust should treat `data/default-config.json` and existing user config as compatibility fixtures.  The rewrite may add new optional fields, but must preserve existing fields, defaults, built-in scene IDs/labels, provider IDs, registry base URL semantics, and validation/normalization behavior.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| D-Bus service ABI | Exact legacy `sd-bus` vtable; void and tuple method signatures; legacy signals. | Same names/paths, zbus facade, but several methods return JSON/string diagnostics. | Signature mismatch, especially `GetAsrBackendState`; no exact introspection contract tests. | Yes | Add D-Bus signature tests first, then adjust zbus interface to legacy signatures; move diagnostic JSON to CLI/new methods. | zbus multiple-return mapping and signal signature details. | P0 |
...
| ASR reload state | target/effective backend, previous backend retained on failure, reload progress, remote endpoints. | Diagnostic `AsrBackendState` derived from config/backend. | No real reload, warmup, previous-effective fallback. | Yes | Add state classification tests and manager abstraction before real backend. | Concurrent reload while recording. | P1 |
| Text/LLM post-processing | OpenAI-compatible HTTP, prompt files, interpolation, context cache, candidate JSON parsing, command mode. | Prompt/request/command-adapter seams; mock/command processors. | No HTTP LLM provider, legacy prompt syntax/context behavior incomplete. | Yes for user config | Add pure prompt/context/candidate tests, then HTTP client with redaction/cancellation. | Secret handling; provider-specific `extra_body`. | P1/P2 |
| Adapter supervision | Start/stop scripts, pid files, working dir, running status. | Text adapter registry and command processor; D-Bus start/stop stubs. | No process supervision or pid lifecycle. | Yes | Implement adapter supervisor in text/daemon boundary; keep D-Bus ABI void. | Process leaks and cross-platform semantics. | P2 |
