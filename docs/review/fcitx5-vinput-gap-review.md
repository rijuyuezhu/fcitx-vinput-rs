...
| Command ASR | Batch raw PCM stdin; streaming line JSON stdout; timeouts/errors. | Legacy batch raw-PCM runner, `.streaming` JSON-line runner, JSON helper seam, process timeout/error tests, and runtime PCM metadata forwarding. | Remaining parity gaps are incremental long-lived streaming sessions, richer legacy adapter edge cases, and broader fixture coverage. | Yes for existing adapters | Keep legacy batch/streaming fixtures pinned; extend streaming session lifecycle and stderr/timeout cancellation cases before treating third-party helpers as drop-in compatible. | Process timeout/cancellation, stderr handling, and helper protocol drift. | P1 |
