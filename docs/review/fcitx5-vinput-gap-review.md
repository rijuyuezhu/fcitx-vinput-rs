...
- Text crate has prompt fixtures, legacy `file:///` prompt-file loading with the 256 KiB cap, whitespace-tolerant `{{ asr }}`/`{{ selected }}`/`{{ context }}` rendering, command adapter seams, and deterministic command processor tests. It still lacks an OpenAI-compatible HTTP LLM client and recent-input context-cache parity.
...
9. OpenAI-compatible post-processing and command mode parity; prompt-file loading and whitespace-tolerant legacy interpolation are now pinned, while HTTP request building, XML fallback wrapping, context cache, and provider call behavior remain.
