...
- Config can parse the current legacy default config, pins committed-file vs bundled-default parity, covers minimal/preserved/partial built-in scene normalization plus omitted/blank active-scene defaulting, and has extensive validation tests.
...
4. Config/default-config compatibility fixtures beyond committed-vs-bundled parity, built-in scene normalization, and omitted/blank active-scene defaulting, plus normalize-vs-validate policy.
