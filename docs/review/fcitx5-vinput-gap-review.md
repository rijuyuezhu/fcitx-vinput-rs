...
| Recognition payload | Stable JSON payload and candidate source semantics. | Matching Rust model, fallback tests, shared raw JSON fixture consumed by protocol/daemon/CLI tests, and protocol-level menu/sentinel goldens. | Need shared daemon/CLI coverage for menu/sentinel fixtures and more legacy parser edge cases. | Yes | Keep raw fixture shared across protocol, D-Bus, and CLI; promote menu/sentinel goldens into daemon/CLI fixtures next; keep serde field names pinned. | Low. | P0 |
...
2. Recognition JSON golden fixtures shared by CLI/daemon/protocol tests; raw is shared across all three, while menu/sentinel remain protocol-level goldens.
...
1. **Lock contracts first**
   ...
   - Promote remaining menu/sentinel recognition payload goldens into shared daemon/CLI fixtures.
