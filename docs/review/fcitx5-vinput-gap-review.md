...
- Registry crate deliberately lacks network download and archive handling; it currently owns pure manifest contracts, zero/missing version validation, deterministic planning, checksum policy, and empty/traversal/absolute path safety fixtures.
...
11. Registry fetch/cache/archive/install/materialization; pure manifest zero/missing version validation, planning, checksum policy, and empty/traversal/absolute path safety fixtures already exist.
