...
| Text/LLM post-processing | OpenAI-compatible HTTP, prompt files, interpolation, context cache, candidate JSON parsing, command mode. | Prompt/request/command-adapter seams; mock/command processors; `file:///` prompt-file loader with 256 KiB cap; whitespace-tolerant legacy double-brace interpolation; OpenAI-compatible endpoint URL/header helpers with redacted request diagnostics, provider-selection processor seam, candidate response/payload mapper, protected `extra_body` merge helper, pure request-body builder with XML fallback/constraints plus command selected-text XML, recent-input context-cache prefix/request helpers, and command-mode OpenAI payload fallback ordering. | No concrete HTTP LLM provider/client or complete runtime orchestration. | Yes for user config | Wire the tested transport seam to a concrete HTTP client and runtime cache path with cancellation/error mapping. | Secret handling; provider-specific `extra_body`; cancellation; cache I/O diagnostics. | P1/P2 |
...
9. OpenAI-compatible post-processing and command mode parity; endpoint URL/header building with redacted request diagnostics, prompt-file loading, whitespace-tolerant legacy interpolation, provider selection, candidate parsing/response-payload mapping, command-mode OpenAI fallback ordering, protected `extra_body` merging, pure request-body assembly with XML fallback/constraints plus command selected-text XML, and recent-input context-prefix helpers are now pinned, while concrete HTTP transport and runtime provider wiring remain.
