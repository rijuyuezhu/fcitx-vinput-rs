...
| Recognition payload | Stable JSON payload and candidate source semantics. | Matching Rust model, fallback tests, shared raw fixture consumed by protocol/daemon/CLI tests, and menu/sentinel fixtures consumed by protocol/CLI tests. | Need daemon/D-Bus coverage for menu/sentinel fixtures and more legacy parser edge cases. | Yes | Keep raw fixture shared across protocol, D-Bus, and CLI; promote menu/sentinel goldens into daemon/D-Bus fixtures next; keep serde field names pinned. | Low. | P0 |
...
2. Recognition JSON golden fixtures shared by CLI/daemon/protocol tests; raw is shared across all three, while menu/sentinel are shared by protocol and CLI but still need daemon/D-Bus fixtures.
...
1. **Lock contracts first**
   ...
   - Promote remaining menu/sentinel recognition payload goldens into daemon/D-Bus fixtures.
