...
Long-running daemon sessions default to `--audio-backend mock`. Use `cargo run -p vinput-daemon --features pipewire-backend -- audio-devices` to inspect the configured PipeWire target and pinned `S16LE`/16 kHz/mono recording plan. Use `cargo run -p vinput-daemon --features pipewire-backend -- --dbus --audio-backend pipewire` only for explicit desktop capture work; the option selects the live PipeWire recorder worker without changing CI or staged mock/configured demos.
