...
`PipeWireStreamConfig` records the selected capture target plus the pinned `S16LE` 16 kHz mono PCM policy that future live streams must request. `PipeWireAudioRecorder` currently exists behind `pipewire-backend` as an explicit skeleton: it stores the selected `PipeWireStreamConfig`, accepts the chunk callback seam, links and initializes the PipeWire client library, and returns `RecordingBackendUnavailable` instead of silently falling back to mock capture. The daemon `audio-devices` diagnostic prints the same planned recording target/format/rate/channel policy so desktop users can verify config before the live stream loop exists. The future live implementation should negotiate signed 16-bit 16 kHz mono PCM first, stream chunks through the callback, and materialize `CapturedAudio` with source metadata on stop.
