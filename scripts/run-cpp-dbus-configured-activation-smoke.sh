...
cmake -S cpp/fcitx5-addon -B "${build_dir}" \
  ...
  -DVINPUT_DAEMON_ARGS="--dbus --configured-backends --config ${config_path}"
...
grep -qx "Exec=${daemon_path} --dbus --configured-backends --config ${config_path}" "${service_file}"

XDG_DATA_DIRS="${repo_root}/${stage_dir}/share:${XDG_DATA_DIRS:-/usr/local/share:/usr/share}" \
VINPUT_DBUS_SMOKE_EXPECTED_NORMAL="demo heard 8 bytes" \
VINPUT_DBUS_SMOKE_EXPECTED_COMMAND="demo final: demo heard 8 bytes" \
