...
fn default_adapter_runtime_dir_prefers_xdg_runtime_dir() {
    assert_eq!(
        ...
        std::path::PathBuf::from("/run/user/1000/vinpst/adapters")
    );
}
...
fn default_adapter_runtime_dir_falls_back_to_temp_dir() {
    assert_eq!(
        ...
        std::env::temp_dir().join("vinpst").join("adapters")
    );
    assert_eq!(
...
fn default_context_cache_path_matches_legacy_xdg_order() {
    assert_eq!(
        ...
        std::path::PathBuf::from("/cache-home/vinpst/context.jsonl")
    );
    assert_eq!(
        default_context_cache_path(None, Some(std::path::Path::new("/home/demo"))),
        std::path::PathBuf::from("/home/demo/.cache/vinpst/context.jsonl")
    );
    assert_eq!(
        default_context_cache_path(
        ...
        std::path::PathBuf::from("vinpst/context.jsonl")
    );
}
...
fn adapter_registry_indexes_command_adapters_from_config() {
    ...
    assert_eq!(adapter.command(), "vinpst-postprocess");
    assert_eq!(adapter.env().get("MODE").map(String::as_str), Some("test"));
    assert_eq!(adapter.working_dir(), Some("/tmp/vinpst"));
...
fn command_text_processor_delegates_to_single_adapter() {
    ...
    assert_eq!(
        payload.commit_text,
        "vinpst-postprocess --json mock /tmp/vinpst: raw text"
    );
}
...
fn command_text_adapter_delegates_to_injected_runner() {
    ...
    assert_eq!(
        payload.commit_text,
        "vinpst-postprocess --json mock /tmp/vinpst: hello"
    );
}
